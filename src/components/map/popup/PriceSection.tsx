...
export default function PriceSection({ unit }: PriceSectionProps) {
  return (
    <div className="flex flex-col gap-1">
      {/* 여러 가격 세트가 있는 경우 */}
      {unit.priceSets && !unit.price && (
        <div className="grid grid-cols-2 gap-1">
          {unit.priceSets.map((priceSet, index) => (
            <div
              key={index}
              className="group flex flex-col gap-0.5 border border-gray-8 hover:border-secondary hover:bg-secondary/5 rounded-md p-1 text-sm"
            >
              {priceSet.label && (
                <div className="text-xs text-gray-3 text-center font-medium pb-0.5 mb-0.5">
                  {priceSet.label}
                </div>
              )}
              {priceSet.deposit && (
